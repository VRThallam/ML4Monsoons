...
from sklearn.ensemble import GradientBoostingRegressor                 # does't work for GPU use EML_GPU.py instead because of the limitation in scikit-learn
...
dataset = loadtxt('/content/feb_1_train_all_param.csv', delimiter=',', skiprows=1)
X = dataset[:, 0:36]
...
# GridSearch to find best parameters for KNN
param_grid = {
...
# Use best estimator for predictions and saving
joblib.dump(best_knn, "ensemble_model_knn.pkl")
...
print(f"Weighted Ensemble MAE: {mae:.4f}")

# ========== Save Predictions ==========
np.savetxt("ensemble_weighted_feb_1_predictions.csv", ensemble_pred, delimiter=",")
np.savetxt("quantile_q10_feb_1.csv", q10_inv, delimiter=",")
np.savetxt("quantile_q50_feb_1.csv", q50_inv, delimiter=",")
np.savetxt("quantile_q90_feb_1.csv", q90_inv, delimiter=",")
np.savetxt("y_test_actual_feb_1.csv", y_test_orig, delimiter=",")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("ensemble_weighted_feb_1_predictions.pdf")
plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("quantile_feb_1_prediction_bands_weighted.pdf")
plt.show()
