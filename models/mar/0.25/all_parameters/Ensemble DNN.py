...
for seed in seeds:
    ...
    mc = ModelCheckpoint(f'Ensemble_DNN_model_seed_mar_1_{seed}.keras', monitor='val_loss', mode='min', save_best_only=True)
...
plt.legend()
plt.grid(True)
plt.savefig('ensemble_dnn_mar_025_val_loss_comparison.pdf')
plt.show()
...
np.savetxt("ensemble_dnn_mar_025_predictions.csv", ensemble_preds_rescaled, delimiter=",")
np.savetxt("ensemble_dnn_mar_025_uncertainty.csv", ensemble_std_rescaled, delimiter=",")
y_test_rescaled = output_scaler.inverse_transform(y_test_scaled)
np.savetxt("ensemble_dnn_mar_025_true_values.csv", y_test_rescaled, delimiter=",")
...
print(f"Ensemble_DNN RMSE: {rmse:.2f}")
...
plt.grid(True)
plt.savefig('ensemble_dnn_mar_025_true_vs_predicted_uncertainty.pdf')
plt.show()
...
plt.grid(True)
plt.savefig('ensemble_dnn_mar_025_residuals_plot.pdf')
plt.show()
