...
dataset = loadtxt('/content/March(0.25)train_data_1982_2018.csv', delimiter=',', skiprows=1)
X = dataset[:, 0:36]
...
gbr = XGBRegressor(
    ...
    learning_rate=0.05,
    max_depth=4,
    tree_method='hist',  # or 'gpu_hist' (both GPU compatible)
    device='cuda',
    random_state=42
)
...
for q in quantiles:
    ...
    gbr_q.fit(x_train_scaled, y_train_scaled.ravel())
...
mae = mean_absolute_error(y_test_orig, meta_pred)
r2 = r2_score(y_test_orig, meta_pred)

print(f"Stacked Ensemble RMSE: {rmse:.2f}")
print(f"Stacked Ensemble MAE : {mae:.2f}")
print(f"Stacked Ensemble R²  : {r2:.3f}")


# ========== Save Predictions ==========
np.savetxt("predictions/stacked_ensemble_mar_025_predictions.csv", meta_pred, delimiter=",")
np.savetxt("predictions/y_test_actual_mar_025.csv", y_test_orig, delimiter=",")
np.savetxt("predictions/quantile_mar_025_q10.csv", q10_inv, delimiter=",")
np.savetxt("predictions/quantile_mar_025_q50.csv", q50_inv, delimiter=",")
np.savetxt("predictions/quantile_mar_025_q90.csv", q90_inv, delimiter=",")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("predictions/stacked_ensemble_mar_025_predictions.pdf")
plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("predictions/quantile_mar_025_prediction_bands_stacked.pdf")
plt.show()
