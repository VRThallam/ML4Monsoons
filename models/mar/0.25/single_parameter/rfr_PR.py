...
# Load dataset
dataset = np.loadtxt('C:\\March(0.25)train_data_only_prec__1982_2018.csv', delimiter=',', skiprows=1)
X = dataset[:, 0:14]
...
# Save best model
joblib.dump(random_search.best_estimator_, 'rf_randomized_best_model_mar_025_only_prec.pkl')
...
# Save predictions
np.savetxt("rf_mar_025_only_prec_predictions.csv", predictions, delimiter=",")
np.savetxt("rf_mar_025_only_prec_y_test.csv", y_test_orig, delimiter=",")
...
print(f"RF MAE: {mae:.4f}, R² Score: {r2:.4f}")
...
plt.savefig("rf_mar_025_only_prec_prediction_vs_true.pdf")
plt.show()
...
plt.savefig("rf_mar_025_only_prec_feature_importance.pdf")
plt.show()
